...
compareReactionCores <- function(model1, model2){
  ...
  return(length(union(setdiff(fd1$Reaction, fd2$Reaction), setdiff(fd2$Reaction, fd1$Reaction))))
...
checkDuplicateReactionCores <- function(generationNo){
  ...
  setDiffReactionsDF <- data.frame(M1 = sapply(rowCombination, `[`, 1),
                             ...
                             commonCoreR = do.call(rbind, difference))
  
  write.table(setDiffReactionsDF, 
              paste0("D:/BellosData/GF_3/compare_ReactionCores/Gen_",generationNo,"_reactionCoreComparison.txt"), 
