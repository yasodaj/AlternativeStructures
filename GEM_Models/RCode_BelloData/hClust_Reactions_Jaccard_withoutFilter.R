...
computerJaccardDissimilarityR <- function(file1, file2){ #file1 = model 1, file2 = model2
  ...
  jaccardSimilarity = length(intersect(fd1$Reaction,fd2$Reaction))/length(union(fd1$Reaction, fd2$Reaction))
...
clusteringWithCommonReactions <- function(generationNo){
  ...
  write.table(similarityDF, 
              paste0("D:/BellosData/GF_3/Clustering_Reactions_Jaccard/WithoutFiltering/Gen_",generationNo,"_jaccardDisimilarityMtrx.txt"), 
  ...
  png(paste0("D:/BellosData/GF_3/Clustering_Reactions_Jaccard/WithoutFiltering/Gen_",generationNo,"_jaccard_hclust.png"), height = 700, width = 1200)
  plot(hcdN, xlab = "Height", horiz = F , 
       main = paste0("Common Reactions with Jaccard dist - Generation ",generationNo))
  dev.off()
