...
dataPlot = read.table("MasterSummary_BadGeneration.txt",sep = ",", header = T)
...
plotAll <- ggplot(meltedData, aes(Index,value, col=variable)) + 
  ...
  ggtitle("Occurance of each gene per generation (Bad Gen)") +
  labs(x = "Generation", y = "Gene Count") +
  theme(legend.position="none") 

ggsave(filename = "AllInOne_BadGen.png", plot=plotAll, width = 15 , height = 7)
...
ggsave(filename = "AllInOne_BadGen_withLegend.png", plot=plotAll2, width = 15 , height = 7)
...
ggsave(filename = "AllInSeperate_BadGen.png", plot = plotSeparate, width = 17 , height = 10)
