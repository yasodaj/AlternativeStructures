...
public class TestMainProg {
	...
	public void go(int iteration, int popSize, String opr) {
		...
		factories.add(new IntegerConstantFactory(1, 9)); //12,60,1 and the target number
		double num = 60;
...
//						  karvaEvaluator.print(bestIndividual));    
					...
					try{
						...
						String fileName = "Target_"+ num +"_popSize_" + popSize + "_Iterations_2000_opr_" + opr + ".txt";
...
//				}
		...
		int elite = (popSize * 10)/100;
		engine.evolve(popSize, elite, new TargetFitness(0.0001, false));
...
//		}
		...
		//String[] operations = {"Add","Div","Sub", "Mult", "Add_Mult", "Add_Mult_Div", "Add_Mult_Div_Sub"};
		String[] operations = {"Add_Mult_Div_Sub"};
		//String[] operations = {"Add"};
		
		String cOperation;
		int cpopSize;
		
		//int[] popSize = {3000,2500,2000,1500,1000,500,400,300,200,100,50,10};
		int[] popSize = {500};
		
		//int[] popSize = {1000,1500,500,2500,3000};
		...
		for(int i=0;i<operations.length;i++){
			...
			for(int j = 0; j < popSize.length; j++){
				...
				for(int k = 0; k < 1; k++){
					...
					new TestMainProg().go(k, cpopSize, cOperation);
				}
			}
