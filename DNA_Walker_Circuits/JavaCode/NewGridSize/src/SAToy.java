...
public class SAToy {
	public static String saveDirectoryParent = "E:\\SA\\Toy\\NewGridSize_10000I\\";
	private static String saveDirectory = "E:\\SA\\Toy\\NewGridSize_10000I\\";
	...
	private static HashMap<String, Double> alternativeSolutions = new HashMap<String, Double>();
	...
	private static double iter = 10000;
	//Convergence point
	private static double cp = -1;
	//x - solution - xnew new solution
	private static int[] initialSolution, newSolution, initialSolutionCopy; 
	//initialSolutionCopy will store a copy of the init solution at the begining of each iteration
	
	public static void main(String args[]) throws Exception {
		
		populateNodeIdentifierToy(); // load node labels for the layout
		populateNodeConnectionIdentifierToy(); // load node connections 
		populateNodeBranchRelationshipToy(); //load node branch connection
		String fitnessFileName = "E:\\TestDavidSolutions\\Old marcie code\\NewSolutions_toy_revised\\Solutions_toy_revised_newFitness"
...
//		saveDirectory = saveDirectoryParent + "Test" + "\\";
		...
		int[] elitex = initialSolution.clone();		
		...
		{
//			if(i == 0) {
//				initialSolution = new int[]{11,1,11,3,8,3,13,4,8,5,13,6};
//			}
			
			initialSolutionCopy = initialSolution.clone();
			
			fileNamePrefix = "SA_Init_Iteration_"+ i;
			fit = calculateFitnessMaster(initialSolution, fileNamePrefix);
			
			
			newSolution = smallChangeMaster(initialSolution);
			fileNamePrefix = "SA_NewSol_Iteration_"+ i;
			fdash = calculateFitnessMaster(newSolution, fileNamePrefix);
			if (fdash <= fit) //maximization problem
			{
				fit = fdash;
				cp = i;
				alternativeSolutions.put(convertArrayToString(newSolution), fit);
				initialSolution = newSolution.clone();
			} 
//			else if(fdash == fit) {
//				
//				if(!alternativeSolutions.containsKey(convertArrayToString(newSolution))) {
//					
//					alternativeSolutions.put(convertArrayToString(newSolution), fit);
//					writeToAlternativeSolutionsLog(i, fdash, newSolution);
//				}
//				
//			}
			else
...
//				System.out.println(i + " " + fit + " " + elitef);
			...
			writeToBestSolutionsLog(i, elitef, elitex, T);
		} // end for
		
		fit = elitef;
		initialSolution = elitex.clone();
	...
	public static int[] generateRandomSolution() {
		...
		//linkedHashMap because it preserves the insertion order
	        ...
	        for(int i = 0; i < 2; i++) {
	        	...
	        	default:
	        		try {
	        			...
	        			if(!populatedNodeIDs.contains(Integer.valueOf(currentConnection[i]))) {
...
//			System.out.println();
	...
	public static double calculateFitnessMaster(int[] solution, String candleFileNamePrefix) {
		...
		fitness = calculateFitnessWithouMarcie(leaks, solution,candleFileNamePrefix);
		
		return fitness;
	}
	
	public static double calculateFitnessWithouMarcie(int[] leaks, int[] solution, String candleFileNamePrefix) {
...
//		writeToSolutionsLog("My-greatest-leaks-function",solution,leaks[0],leaks[1],leaks[2],area,fitness);
		writeToSolutionsLog(candleFileNamePrefix,solution,leaks[0],leaks[1],leaks[2],area,fitness);
...
//		stringArray += "]";
	...
	public static void populateNodeConnectionIdentifierToy() {
		
		nodeConnectionIdentifier.put(new int[] {0,1},"Init-Fork");
		nodeConnectionIdentifier.put(new int[] {1,2},"Fork-Norm1");
		nodeConnectionIdentifier.put(new int[] {1,3},"Fork-Norm2");
		nodeConnectionIdentifier.put(new int[] {2,4},"Norm1-Final1");
		nodeConnectionIdentifier.put(new int[] {3,5},"Norm2-Final2");
	...
	public static void writeToBestSolutionsLog(int iteration, double elitef, int[] elitex, double temp) {
		...
		try
		{
		    ...
		    fw.write(String.valueOf(iteration) + "\t" + df.format(fit) + "\t" + df.format(elitef) +
		    		...
		    		convertArrayToString(initialSolutionCopy) + "\t" + convertArrayToString(newSolution) +
		    		"\t"+ String.valueOf(temp) + "\n");
		    fw.close();
		}
		catch(IOException ioe)
	...
	public static void writeToAlternativeSolutionsLog(int iteration, double fit, int[] solution) {
		
		DecimalFormat df = new DecimalFormat("#.##");
		
		try
		{
		    String logFilename= saveDirectory + "AlternativeSolutionsLog.txt";
		    FileWriter fw = new FileWriter(logFilename,true); //the true will append the new data
		    fw.write(String.valueOf(iteration) + "\t" + convertArrayToString(solution) + "\t" +
		    		df.format(fit) + "\n");
		    fw.close();
		}
		catch(IOException ioe)
	...
	public static void populateNodeBranchRelationshipToy() {
		...
		nodeBranchRelationship.put(Integer.valueOf(3),Integer.valueOf(2));
		nodeBranchRelationship.put(Integer.valueOf(4),Integer.valueOf(1));
